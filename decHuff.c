...
void readMap(unsigned char *buffer, int largestEncodingLen, FILE *pFile, int lengths[], short inputSize)
{
...
void orderMapAndRebuildEncodings(int *lengths, int *keysOfMap, struct Node *tree, short inputSize)
{
    int stop=1;
    do
    {
    ...
    for(int q=0;q<pow(2, inputSize);q++)
    {
        if(lengths[q]==0)
            continue;
            //in either case length[q] is the first char present
...
void decompress(struct Node *tree, unsigned char buffer[], FILE *pFile, int largestEncodingLen, int fileLen, short bitsToSkip, int uncompressed, short inputSize, short nameIndex)
{
    unsigned char *outputNames[]={{"_flags.lztmp"},{"_lengths.lztmp"},{"_next.lztmp"},{"_offsets.lztmp"}};
    ...
    while(nElemRead=fread(buffer, 1, 100, pFile))
    {
        for(int byteI=0;byteI<nElemRead;byteI++, absByteCounter++)
        {
            ...
            if(absByteCounter+largestEncodingLen*pow(2, inputSize)/8>fileLen-1-4-bytesToSkip)
            {
            ...
            for(int bitI=0;bitI<strlen(bit) && absByteCounter+largestEncodingLen*pow(2, inputSize)/8<=fileLen-1-4-bytesToSkip;bitI++)
            {
                if(8-bitI==bitsToSkip && absByteCounter+largestEncodingLen*pow(2, inputSize)/8==fileLen-1-4-bytesToSkip)//-2 for the header
                    break;
                if(bit[bitI]=='0')
                {
                    dummy=(*dummy).left;
...
int decompressHuffman(char inputName[])
{
	...
	for (short nameIndex = 0; nameIndex < 4; nameIndex++) {
		...
		//initially I need decimal length
